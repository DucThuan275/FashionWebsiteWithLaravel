<x-layout-admin>
    ...
    <div class="container mx-auto mt-4">
        <h1 class="text-xl font-bold mb-4">Chỉnh Sửa Bài Viết</h1>
        ...
        <form action="{{ route('post.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="mb-2">
                ...
                <input type="text" id="title" class="w-full border rounded p-2" name="title"
                    value="{{ old('title', $post->title) }}" placeholder="Nhập tiêu đề bài viết">
                ...
                <select id="topic_id" name="topic_id" class="w-full border rounded p-2">
                    ...
                    @foreach ($topics as $topic)
                        <option value="{{ $topic->id }}"
                            {{ old('topic_id', $post->topic_id) == $topic->id ? 'selected' : '' }}>{{ $topic->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-2">
                ...
                <textarea id="description" name="description" rows="4" class="w-full border rounded p-2"
                    placeholder="Nhập mô tả ngắn cho bài viết">{{ old('description', $post->description) }}</textarea>
            </div>
            ...
            <div class="mb-2">
                ...
                <select id="status" name="status" class="w-full border rounded p-2">
                    ...
                    <option value="0" {{ old('status', $post->status) == 0 ? 'selected' : '' }}>Không Kích Hoạt
                    </option>
                </select>
            </div>
