            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Chủ
                            Đề
                        </th>
                    ...
                    @foreach ($posts as $post)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{ \Illuminate\Support\Str::limit($post->description, 10) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $post->topic->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <input type="checkbox" class="peer sr-only opacity-0"
                                            id="toggle-{{ $post->id }}"
                                            onchange="window.location.href='{{ route('admin.post.status', ['post' => $post->id, 'id' => $post->id]) }}'"
                                            {{ $post->status ? 'checked' : '' }} />
